...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf(AbstractHttpConfigurer::disable) //Not needed since we use stateless JWT token authentication
                ...
                .authorizeHttpRequests(request -> request
                        .requestMatchers(new RegexRequestMatcher("/user/get/.*", HttpMethod.GET.name())).hasAnyAuthority(ADMIN.getValue(), STANDARD.getValue())
                        .requestMatchers(new RegexRequestMatcher("/user/save", HttpMethod.POST.name())).hasAnyAuthority(ADMIN.getValue(), STANDARD.getValue())
                        .requestMatchers(new RegexRequestMatcher("/user/update/.*", HttpMethod.PUT.name())).hasAnyAuthority(ADMIN.getValue(), STANDARD.getValue())
                        .requestMatchers(new RegexRequestMatcher("/user/delete/.*", HttpMethod.DELETE.name())).hasAnyAuthority(ADMIN.getValue(), STANDARD.getValue())
                )
